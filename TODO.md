...
- [x] **Ingest compounding** — _compound_wiki_pages runs after stale refresh during post-ingest. Extracts entities (resolved through EntityIndex to canonicals) and concept tags (slug-normalized) from new chunks, finds overlapping existing pages, regenerates with force=True. Knowledge compounds immediately on ingest.
